    ...
    switch flag
      ...
      case 310
        if (NF > NFTEST)
          NF = NF - 1;
          if (IPRINT > 0)
            warning('return from NEWUOA because CALFUN has been called MAXFUN times.');
          end
          flag = 530;
        else
          F = Func(N, X);
          ...
          elseif (KNEW == -1)
            ...
            elseif (VQUAD >= ZERO)
              if (IPRINT > 0)
                warning('return from NEWUOA because a trust region step has failed to reduce Q.')
              end
              flag = 530;
            else
              RATIO = (F - FSAVE) / VQUAD;
      ...
      case 410
        [BMAT_, ZMAT_, IDZ_, VLAG_, W_] = ...
          updatelilin (N, NPT, BMAT, ZMAT, IDZ, VLAG, BETA, KNEW, W);
        W = W_;
        VLAG = VLAG_;
        BMAT = BMAT_;
        ZMAT = ZMAT_;
